...
public class ServiceBusTrigger
{
    ...
    {
        _logger.LogInformation("PROCESS RED : SERVICE BUS TRIGGER : RECIEVED : ITEM: {queueItem}", myQueueItem);

        var appPropertiesJson = context.BindingContext.BindingData["ApplicationProperties"];

        _logger.LogInformation("PROCESS RED : SERVICE BUS TRIGGER : RECIEVED : APP PROPS: {appPropsJson}", appPropertiesJson);

        var appPropertiesDict = context.GetServiceBusMessageHeaders();

        _logger.LogInformation("PROCESS RED : SERVICE BUS TRIGGER : HEADER CONTENT : COLOR : {colorValue}", appPropertiesDict["color"]);

        var blobContent = $"{appPropertiesJson}";

        _logger.LogInformation("PROCESS RED : SERVICE BUS TRIGGER : BLOB PAYLOAD : {blobContent}", blobContent);
